...
function check_email()
{
    ...
    if(obj==""||!/^\w(\w*\.*)*@(\w+\.)+\w{2,4}$/.test(obj))
    {
        ...
        // alert("邮箱格式不正确!");
        // document.addUserForm.juemail.focus();
        return false;
    } else {
...
function forgetpassword() {
    ...
    console.log(obj)
    $.ajax({
        url:"http://localhost:8080/popo_war_exploded/user/forget/modify",//这里是你接口的路径，记住http://不要忘记,localhost是你本地的地址，如果要请求别人的请将别人的ip地址替换，后面的addInfoPerson是接口名字。
        type:"POST",//请求方式是get还是post,我这里用的是post
        contentType:"application/json",
        data:JSON.stringify(obj),//传递的参数
        dataType:"json",
        success:function(data){ //成功返回的回调函数
            if((data).modifyInfo == "修改失败") {
